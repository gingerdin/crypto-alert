...
import bot.telegram.impl.commands.ClearStateCommand;
...
public class CryptoRateCommand extends DefaultBotCommand {
    ...
    public CryptoRateCommand(BotNameProvider botNameProvider, TelegramLongPollingBot sender) {
    ...
    protected String executeAction(Update update) {
        State state = DatabaseManager.getInstance().getBotState(sender.getBotUsername(), update.getMessage().getChat().getUserName(), update.getMessage().getChatId());
